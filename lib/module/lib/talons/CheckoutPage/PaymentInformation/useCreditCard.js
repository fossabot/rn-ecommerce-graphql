...
export const useCreditCard = props => {
  ...
  } = useQuery(getBillingAddressQuery, {
    skip: !cartId,
    variables: {
      cartId
    }
  ...
  } = useQuery(getShippingAddressQuery, {
    skip: !cartId,
    variables: {
      cartId
    }
  ...
  } = useQuery(getIsBillingAddressSameQuery, {
    skip: !cartId,
    variables: {
      cartId
    }
  ...
  const setPaymentDetailsInCache = useCallback(braintreeNonce => {
    /**
  ...
  const updateCCDetailsOnCart = useCallback(braintreeNonce => {
    const {
      nonce
    } = braintreeNonce;
    updateCCDetails({
  ...
  const errors = useMemo(() => new Map([['setBillingAddressMutation', billingAddressMutationError], ['setCreditCardDetailsOnCartMutation', ccMutationError]]), [billingAddressMutationError, ccMutationError]);
  return {
    errors,
    onPaymentError,
    onPaymentSuccess,
