...
export const useQuantity = props => {
  ...
  const {
    value: quantity
  } = useFieldState('quantity');
  const isIncrementDisabled = useMemo(() => !quantity, [quantity]); // "min: 0" lets a user delete the value and enter a new one, but "1" is
  // actually the minimum value we allow to be set through decrement button.

  const isDecrementDisabled = useMemo(() => !quantity || quantity <= 1, [quantity]); // Fire the onChange after some wait time. We calculate the current delay
  ...
  const maskInput = useCallback(value => {
    try {
