...
export const useSearchPage = props => {
  ...
  const previousSort = useRef(currentSort); // get the URL query parameters.
  ...
  const previousSearch = useRef(search); // Set up pagination.
  ...
  } = useQuery(filterIntrospection); // Create a type map we can reference later to ensure we pass valid args
  ...
  useEffect(() => {
    ...
    const filters = getFiltersFromSearch(search); // Construct the filter arg object.
  ...
  const [getFilters, {
    data: filterData
  }] = useLazyQuery(getProductFiltersBySearch);
  useEffect(() => {
    if (inputText) {
      getFilters({
        variables: {
