...
import { sendMessageToSW, VALID_SERVICE_WORKER_ENVIRONMENT } from '@magento/venia-ui/lib/util/swUtils';
  ...
  useEffect(() => {
    if (VALID_SERVICE_WORKER_ENVIRONMENT) {
      const urls = images.map(({
        file
      }) => new URL(generateUrlFromContainerWidth(file, imageWidth, type), location.origin).href);
