...
import { DELIMITER } from '../FilterModal/helpers'; // TODO: derive from store config when available
...
const getLocation = (searchValue, categoryId, label) => {
  ...
  uri.searchParams.set('category_id[filter]', "".concat(label).concat(DELIMITER).concat(categoryId));
  const {
    pathname,
    search
