...
export const signOut = (payload = {}) => async function thunk(dispatch) {
  const {
    revokeToken
  } = payload;
  ...
  await clearCheckoutDataFromStorage(); // Now that we're signed out, forget the old (customer) cart.
...
export const resetPassword = ({
  ...
  const [dispatch] = args;
  dispatch(actions.resetPassword.request()); // TODO: actually make the call to the API.
  // For now, just return a resolved promise.

  await Promise.resolve(email);
  dispatch(actions.resetPassword.receive());
};
export const setToken = token => async function thunk(...args) {
...
export const clearToken = () => async function thunk(...args) {
