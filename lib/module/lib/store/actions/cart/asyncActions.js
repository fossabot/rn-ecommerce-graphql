...
export const createCart = payload => async function thunk(dispatch, getState) {
  const {
    fetchCartId
  } = payload;
  const {
  ...
  dispatch(actions.getCart.request()); // if a cart exists in storage, act like we just received it
...
export const getCartDetails = payload => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    try {
      ...
      dispatch(actions.getDetails.receive({
        details
      }));
    } catch (error) {
...
export const removeCart = () => async function thunk(dispatch) {
