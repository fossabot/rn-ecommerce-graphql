...
exports.clearCheckoutDataFromStorage = exports.formatAddress = exports.createAccount = exports.submitOrder = exports.submitShippingMethod = exports.submitShippingAddress = exports.submitPaymentMethod = exports.submitBillingAddress = exports.submitPaymentMethodAndBillingAddress = exports.resetReceipt = exports.resetCheckout = exports.cancelCheckout = exports.beginCheckout = void 0;
...
const beginCheckout = () => async function thunk(dispatch) {
  // Before we begin, reset the state of checkout to clear out stale data.
  dispatch(_actions.default.reset());
  ...
  dispatch(_actions.default.begin({
...
const cancelCheckout = () => async function thunk(dispatch) {
  dispatch(_actions.default.reset());
};

exports.cancelCheckout = cancelCheckout;

const resetCheckout = () => async function thunk(dispatch) {
  await dispatch((0, _app.closeDrawer)());
  dispatch(_actions.default.reset());
};

exports.resetCheckout = resetCheckout;

const resetReceipt = () => async function thunk(dispatch) {
  await dispatch(_actions.default.receipt.reset());
};

exports.resetReceipt = resetReceipt;

const submitPaymentMethodAndBillingAddress = payload => async function thunk(dispatch) {
  const {
    countries,
    formValues
  } = payload;
  ...
  return Promise.all([dispatch(submitBillingAddress({
    billingAddress,
    countries
  })), dispatch(submitPaymentMethod(paymentMethod))]);
};

exports.submitPaymentMethodAndBillingAddress = submitPaymentMethodAndBillingAddress;

const submitBillingAddress = payload => async function thunk(dispatch, getState) {
  dispatch(_actions.default.billingAddress.submit());
  const {
    cart
  } = getState();
  const {
  ...
  try {
    ...
    dispatch(_actions.default.billingAddress.accept(desiredBillingAddress));
  } catch (error) {
    dispatch(_actions.default.billingAddress.reject(error));
    throw error;
  }
};

exports.submitBillingAddress = submitBillingAddress;

const submitPaymentMethod = payload => async function thunk(dispatch, getState) {
  dispatch(_actions.default.paymentMethod.submit());
  const {
    cart
  } = getState();
  const {
  ...
  try {
    ...
    dispatch(_actions.default.paymentMethod.accept(payload));
  } catch (error) {
    dispatch(_actions.default.paymentMethod.reject(error));
    throw error;
  }
};
...
const submitShippingAddress = (payload = {}) => async function thunk(dispatch, getState) {
  dispatch(_actions.default.shippingAddress.submit());
  const {
    formValues,
    countries,
    setGuestEmail,
  ...
  try {
    ...
    const shippingMethods = data.setShippingAddressesOnCart.cart.shipping_addresses[0].available_shipping_methods; // On success, save to local storage.

    await saveAvailableShippingMethods(shippingMethods);
    await saveShippingAddress(address);
    dispatch(_actions.default.getShippingMethods.receive(shippingMethods));
    dispatch(_actions.default.shippingAddress.accept(address));
  } catch (error) {
    dispatch(_actions.default.shippingAddress.reject(error));
    throw error;
  }
};

exports.submitShippingAddress = submitShippingAddress;

const submitShippingMethod = payload => async function thunk(dispatch, getState) {
  dispatch(_actions.default.shippingMethod.submit());
  const {
    cart
  } = getState();
  const {
  ...
  try {
    ...
    dispatch(_actions.default.shippingMethod.accept(desiredShippingMethod));
  } catch (error) {
    dispatch(_actions.default.shippingMethod.reject(error));
    throw error;
  }
};

exports.submitShippingMethod = submitShippingMethod;

const submitOrder = ({
  fetchCartId
}) => async function thunk(dispatch, getState) {
  dispatch(_actions.default.order.submit());
  const {
    cart,
    user
  } = getState();
  ...
  try {
    ...
    const guestShippingEndpoint = "/rest/V1/guest-carts/".concat(cartId, "/shipping-information");
    ...
    const guestPaymentEndpoint = "/rest/V1/guest-carts/".concat(cartId, "/payment-information");
    ...
    dispatch(_actions.default.receipt.setOrder({
      id: response,
      shipping_address
    ...
    dispatch(_actions.default.order.accept());
  } catch (error) {
    dispatch(_actions.default.order.reject(error));
    throw error;
  }
};
...
const createAccount = ({
  ...
  history.push("/create-account?".concat(new URLSearchParams(accountInfo)));
};
/* helpers */
...
const clearCheckoutDataFromStorage = async () => {
...
exports.clearCheckoutDataFromStorage = clearCheckoutDataFromStorage;
