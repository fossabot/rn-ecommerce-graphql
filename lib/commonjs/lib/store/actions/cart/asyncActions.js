...
exports.retrieveCartId = retrieveCartId;
exports.saveCartId = saveCartId;
exports.clearCartId = clearCartId;
exports.writeImageToCache = writeImageToCache;
exports.removeCart = exports.getCartDetails = exports.removeItemFromCart = exports.updateItemInCart = exports.addItemToCart = exports.createCart = void 0;
...
const createCart = payload => async function thunk(dispatch, getState) {
  const {
    fetchCartId
  } = payload;
  const {
    cart
  } = getState(); // if a cart already exists in the store, exit
  ...
  dispatch(_actions.default.getCart.request()); // if a cart exists in storage, act like we just received it

  const cartId = await retrieveCartId();

  if (cartId) {
    dispatch(_actions.default.getCart.receive(cartId));
    return;
  }
  ...
  try {
    ...
    if (errors) {
      ...
      receivePayload = data.cartId; // write to storage in the background

      saveCartId(data.cartId);
    }

    dispatch(_actions.default.getCart.receive(receivePayload));
  } catch (error) {
    // If we are unable to create a cart, the cart can't function, so
    // we forcibly throw so the upstream actions won't retry.
    dispatch(_actions.default.getCart.receive(error));
...
const addItemToCart = (payload = {}) => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    dispatch(_actions.default.addItem.request(payload));
    const {
      cart,
      user
    } = getState();
    ...
    try {
      ...
      dispatch(_actions.default.addItem.receive());
    } catch (error) {
      dispatch(_actions.default.addItem.receive(error));
      const shouldRetry = !error.networkError && isInvalidCart(error); // Only retry if the cart is invalid or the cartId is missing.
...
const updateItemInCart = (payload = {}) => {
  const {
    cartItemId,
    fetchCartDetails,
    fetchCartId,
  ...
  return async function thunk(dispatch, getState) {
    await writingImageToCache;
    dispatch(_actions.default.updateItem.request(payload));
    const {
      cart,
      user
    } = getState();
    ...
    try {
      ...
      dispatch(_actions.default.updateItem.receive());
    } catch (error) {
      dispatch(_actions.default.updateItem.receive(error));
      ...
      if (shouldRetry) {
        ...
        // complete before dispatching the error--you don't want an
        // upstream action to try and reuse the known-bad ID.
        await dispatch(removeCart()); // then create a new one
...
const removeItemFromCart = payload => {
  ...
  return async function thunk(dispatch, getState) {
    dispatch(_actions.default.removeItem.request(payload));
    const {
      cart
    } = getState();
    const {
    ...
    try {
      ...
      dispatch(_actions.default.removeItem.receive());
    } catch (error) {
      dispatch(_actions.default.removeItem.receive(error));
...
const getCartDetails = payload => {
  ...
  return async function thunk(dispatch, getState) {
    ...
    const {
      isSignedIn
    } = user; // if there isn't a cart, create one then retry this operation
    ...
    dispatch(_actions.default.getDetails.request(cartId));
    ...
    try {
      ...
      dispatch(_actions.default.getDetails.receive({
        details
      }));
    } catch (error) {
...
const removeCart = () => async function thunk(dispatch) {
  // Clear the cartId from local storage.
  await clearCartId(); // Clear the cart info from the redux store.
...
async function writeImageToCache(item = {}) {
  ...
  if (sku) {
    ...
    if (image) {
      const imageCache = await retrieveImageCache(); // if there is an image and it differs from cache
