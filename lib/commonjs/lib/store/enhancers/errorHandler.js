...
exports.default = createErrorHandlingStore;

var _app = _interopRequireDefault(require("../actions/app"));

var _createErrorRecord = _interopRequireDefault(require("../../util/createErrorRecord"));
 ...
 *
 */
const APP_DISMISS_ERROR = _app.default.markErrorHandled.toString();
/**
...
function sliceHandledError(state, error) {
  const foundEntry = Object.entries(state).find(([, slice]) => typeof slice === 'object' && // A slice is considered to have "handled" the error if it
...
function errorReducer(state, action) {
  ...
  const {
    type,
    payload
  } = action; // The `error` property should be boolean and the payload is the error
  ...
  if (type === APP_DISMISS_ERROR) {
    const errorsMinusDismissed = unhandledErrors.filter(record => record.error !== error); // If the array is the same size, then the error wasn't here
  ...
  if (!sliceHandled) {
    ...
    const allErrors = [// Dedupe errors in case this one is dispatched repeatedly
    ...new Set(unhandledErrors).add((0, _createErrorRecord.default)(error, // `errorRecord()` requires the window argument for
    // testability, through injection of the
    // non-idempotent Date and Math methods for IDs.
    window, // Also call `errorRecord()` with the current
...
function wrapReducerWithErrorHandling(rootReducer) {
  return function errorHandlingRootReducer(state = {}, action) {
    ...
    nextState.unhandledErrors = unhandledErrors; // Apply errorReducer in the context of this root reducer,
