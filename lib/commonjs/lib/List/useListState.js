...
const useListState = ({
  ...
  const initialState = (0, _react.useMemo)(() => getInitialState({
    getItemKey,
    initialSelection,
    selectionModel
  ...
  const {
    selectedKeys
  } = state; // Whenever the selectedKeys changes, notify.
  ...
  const removeFocus = (0, _react.useCallback)(() => dispatch({
    type: 'REMOVE_FOCUS'
  }), []);
  /**
  ...
  const setFocus = (0, _react.useCallback)(key => dispatch({
    type: 'SET_FOCUS',
    payload: {
      key
    }
  ...
  const updateSelectedKeys = (0, _react.useCallback)(key => dispatch({
