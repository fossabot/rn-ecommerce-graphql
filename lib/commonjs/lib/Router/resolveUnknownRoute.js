...
exports.default = resolveUnknownRoute;
 ...
 */
const persistence = new _simplePersistence.default();
/**
...
function getRouteCacheKey(store) {
  return 'urlResolver' + (store ? "_".concat(store) : '');
...
async function resolveUnknownRoute(opts) {
  const {
    route,
    apiBase,
    store
...
function remotelyResolveRoute(opts) {
  const urlResolve = persistence.getItem(getRouteCacheKey(opts.store)); // If it exists in localStorage, use that value
...
function fetchRoute(opts) {
  ...
  return fetch(url, {
    ...
    if (res.errors) {
      throw new Error("urlResolver query failed: ".concat(JSON.stringify(res.errors, null, 2)));
    }

    const routes = persistence.getItem(getRouteCacheKey(opts.store)) || {};
    routes[opts.route] = res;
    persistence.setItem(getRouteCacheKey(opts.store), routes, 86400); // entire route cache has a TTL of one day
