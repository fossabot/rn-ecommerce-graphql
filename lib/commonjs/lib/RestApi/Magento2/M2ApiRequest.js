...
var _M2ApiResponseError = _interopRequireDefault(require("./M2ApiResponseError"));

var MulticastCache = _interopRequireWildcard(require("./MulticastCache"));
...
class M2ApiRequest {
  constructor(resourceUrl, opts = {}) {
    const storage = new _util.BrowserPersistence();
    const signin_token = storage.getItem('signin_token');
    this.controller = new AbortController();
    this.resourceUrl = resourceUrl; // merge headers specially
    ...
    this.opts = {
      ...
      headers: withDefaultHeaders(new Headers({
        authorization: signin_token ? "Bearer ".concat(signin_token) : ''
      }))
    };
  }
  /**
  ...
  _isMulticastable() {
    return this.opts.hasOwnProperty('multicast') ? this.opts.multicast : !(this.opts.method === 'POST' && this.opts.body);
  }
  /**
  ...
  _fetch() {
    return this._transport(this.resourceUrl, this.opts).then( // When the network operation completes, remove from cache
    // as a side effect.
    res => {
      ...
      if (!response.ok) {
        return response // The response may or may not be JSON.
        // Let M2ApiResponseError handle it.
        .text() // Throw a specially formatted error which
        // includes the original context of the request,
        // and formats the server response.
        .then(bodyText => {
          throw new _M2ApiResponseError.default({
  ...
  _fetchMulticast() {
    ...
    const inflightMatch = MulticastCache.match(this); // Is this request meant to override an existing inflight request?
...
function request(resourceUrl, opts) {
...
//# sourceMappingURL=M2ApiRequest.js.map