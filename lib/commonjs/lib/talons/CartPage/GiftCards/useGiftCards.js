...
const useGiftCards = props => {
  ...
  } = props; // We need the cartId for all of our queries and mutations.
  ...
  const appliedCardsResult = (0, _client.useQuery)(appliedCardsQuery, {
  ...
  const [checkCardBalance, balanceResult] = (0, _client.useLazyQuery)(cardBalanceQuery, {
    // For security, always fetch this from the network and never cache the
    // result.
    fetchPolicy: 'no-cache'
  });
  const [applyCard, applyCardResult] = (0, _client.useMutation)(applyCardMutation);
  const [removeCard, removeCardResult] = (0, _client.useMutation)(removeCardMutation);
  /*
   *  useState hooks.
   */

  const [formApi, setFormApi] = (0, _react.useState)();
  const [mostRecentAction, setMostRecentAction] = (0, _react.useState)(null);
  /*
   * useCallback hooks.
   */

  const applyGiftCard = (0, _react.useCallback)(async () => {
  ...
  const checkGiftCardBalance = (0, _react.useCallback)(() => {
  ...
  const removeGiftCard = (0, _react.useCallback)(async giftCardCode => {
  ...
  const shouldDisplayCardBalance = mostRecentAction === actions.CHECK_BALANCE && Boolean(balanceResult.data); // We should only display the last card error if the most recent action was apply or check and they had an error
...
exports.useGiftCards = useGiftCards;
//# sourceMappingURL=useGiftCards.js.map