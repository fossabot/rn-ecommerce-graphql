...
exports.useShippingMethods = void 0;
...
const useShippingMethods = props => {
  ...
  const {
    data
  } = (0, _client.useQuery)(getShippingMethodsQuery, {
  ...
  const [isShowingForm, setIsShowingForm] = (0, _react.useState)(false);
  const showForm = (0, _react.useCallback)(() => setIsShowingForm(true), []);
  (0, _react.useEffect)(() => {
  ...
  if (data) {
    ...
    if (shippingAddresses.length) {
      ...
      if (shippingMethod) {
        selectedShippingMethod = "".concat(shippingMethod.carrier_code, "|").concat(shippingMethod.method_code);
      }
    }
  }
