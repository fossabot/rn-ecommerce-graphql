...
const useProduct = props => {
  ...
  }] = (0, _client.useMutation)(updateItemQuantityMutation);
  (0, _react.useEffect)(() => {
  ...
  const [isFavorite, setIsFavorite] = (0, _react.useState)(false); // Use local state to determine whether to display errors or not.
  // Could be replaced by a "reset mutation" function from apollo client.
  // https://github.com/apollographql/apollo-feature-requests/issues/170

  const [displayError, setDisplayError] = (0, _react.useState)(false);
  const derivedErrorMessage = (0, _react.useMemo)(() => {
    return displayError && (0, _deriveErrorMessage.deriveErrorMessage)([updateError, removeItemError]) || '';
  }, [displayError, removeItemError, updateError]);
  const handleToggleFavorites = (0, _react.useCallback)(() => {
    setIsFavorite(!isFavorite);
  }, [isFavorite]);
  const handleEditItem = (0, _react.useCallback)(() => {
    setActiveEditItem(item);
    toggleDrawer('product.edit'); // If there were errors from removing/updating the product, hide them
  ...
  const handleRemoveFromCart = (0, _react.useCallback)(() => {
    try {
      removeItem({
        variables: {
          cartId,
  ...
  const handleUpdateItemQuantity = (0, _react.useCallback)(async quantity => {
    try {
