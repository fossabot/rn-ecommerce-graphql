...
function errorRecord(error, window, context, customStack) {
  ...
  const {
    ...
    message,
    name
  } = error; // Reasonably unique, yet readable error ID.

  const seconds = new Date().getSeconds();
  const randomChar = Math.random().toString(36).slice(2, 3).toUpperCase();
  record.id = (constructor && constructor.name || name) + seconds + randomChar; // Add offending line, if possible.
  ...
  if (messageStart > -1) {
    ...
    record.loc = stack.slice(traceStart).replace(window.location.origin, '').trim().split('\n')[0];
  }

  errorRecords.set(error, record); // In development mode, React logs these already.
  // Log in production mode so that users can give Support helpful debug info.

  /* istanbul ignore next */

  if (process.env.NODE_ENV === 'production') {
    logError("%cUnhandled ".concat(record.id), 'background: #CC0000; color: white; padding: 0.1em 0.5em', stack);
  }

  return record;
}
//# sourceMappingURL=createErrorRecord.js.map