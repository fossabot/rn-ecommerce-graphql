...
const IntlPatches = {
  formatToPartsPatch({
    ...
    const format = intlFormats[currency] || { ...intlFormats.USD,
      symbol: currency
    };
    const {
      symbol,
    ...
    if (useGrouping !== false) {
      ...
      if (groups) {
        intParts.push(...groups.map(intPart => JSON.stringify({
          type: 'integer',
          value: intPart
        })));
      }

      const groupDelimJSON = ',' + JSON.stringify({
        type: 'group',
        value: groupDelim
      }) + ',';
      const intAndGroupParts = JSON.parse("[".concat(intParts.join(groupDelimJSON), "]"));
